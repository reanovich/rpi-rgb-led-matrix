...
class RevZArrangementMapper : public PixelMapper {
public:
  RevZArrangementMapper() : parallel_(1) {}

  virtual const char *GetName() const { return "Reverse-Z-mapper"; }

  virtual bool SetParameters(int chain, int parallel, const char *param) {
    if (chain < 2) {  // technically, a chain of 2 would work, but somewhat pointless
      fprintf(stderr, "Reverse-Z-mapper: need at least --led-chain=4 for useful folding\n");
      return false;
    }
    if (chain % 2 != 0) {
      fprintf(stderr, "Reverse-Z-mapper: Chain (--led-chain) needs to be divisible by two\n");
      return false;
    }
    parallel_ = parallel;
    return true;
...
static MapperByName *CreateMapperMap() {
  ...
  RegisterPixelMapperInternal(result, new RevZArrangementMapper());
  return result;
}
